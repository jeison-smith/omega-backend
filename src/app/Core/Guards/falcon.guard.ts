import { ActivatedRouteSnapshot, CanActivate, CanActivateFn, CanMatch, Route, Router, RouterStateSnapshot, UrlSegment } from '@angular/router';
...
export class falconGuard implements CanActivate, CanMatch{
  ...
  private isAuthenticated(): Observable<boolean> {
    return this.authService.userInfo$.pipe(
      ...
      tap((isAuthenticated) => {
        if (!isAuthenticated) {
          this.router.navigate(['/home/accesoDenegado']); // Redirige al login si no está autenticado
        }
      })
    );
  }
